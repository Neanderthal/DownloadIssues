...
uv run download-issues               # legacy: sync all open issues
```
...
- `download_issues.py` - legacy issue downloader (still works independently)
...
- Metadata stored as HTML comment in first issue comment: `<!-- DT-METADATA {...} DT-METADATA -->`
